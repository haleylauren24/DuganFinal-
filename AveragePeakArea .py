...
#taking in a temp file if the second argument is not given.
#fn = "Dugan_MSdata.csv"
fn = sys.argv[1]   
...
conds2 = sys.argv[2]
conds3 = sys.argv[3]
condAr.append(conds2)
condAr.append(conds3)


#Splitting up the peak areas and corresponding properties from Skyline
#into a list of lists based on the product mass to charge ratio because
#this is what varies between the transitions
while k<len(lprecs):
...
while w<len(KOthenWT):
    ...
    while y<len(KOthenWT[w]):
        ...
        while x<len(KOthenWT[w][y]):
            #If area in input file is not equal to N/A or 0 then append it
